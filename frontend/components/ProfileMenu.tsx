...
export function ProfileMenu({ user, onChange }: { user: User | null; onChange: (user: User | null) => void }) {
  ...
  return <div className="relative">
    <button aria-label="Profile" onClick={() => setOpen(!open)} className="glass flex h-11 w-11 items-center justify-center rounded-full text-sm font-semibold active:scale-[0.96]">
      {initials || <UserRound size={19} className="text-[#8E8E93]" />}
    </button>
