...
export function CategoryPickerSheet({
  ...
  return <Sheet title="Choose category" onClose={onClose}>
    ...
    <div className="grid grid-cols-2 gap-3">{visible.map(({ name, icon: Icon }) => <button key={name} onClick={() => { onSelect(name); onClose(); }} className={`glass flex items-center gap-3 rounded-2xl p-4 text-left transition-transform duration-150 active:scale-[0.96] ${value === name ? "ring-1 ring-white/50" : ""}`}><Icon size={19} />{name}{suggestions.includes(name) && <Star size={14} className="ml-auto text-[#FFD60A]" aria-label="Suggested" />}</button>)}</div>
  </Sheet>;
}
