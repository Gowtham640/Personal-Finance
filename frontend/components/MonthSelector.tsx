...
export function MonthSelector({ month, onChange }: { month: Date; onChange: (month: Date) => void }) {
  ...
  return <div className="flex min-w-0 items-center justify-center gap-2">
    <button type="button" aria-label="Previous month" onClick={(event) => { event.stopPropagation(); move(-1); }} className="rounded-full p-2 text-[#8E8E93] hover:bg-white/10"><ChevronLeft /></button>
    ...
    <button type="button" aria-label="Next month" onClick={(event) => { event.stopPropagation(); move(1); }} className="rounded-full p-2 text-[#8E8E93] hover:bg-white/10"><ChevronRight /></button>
  </div>;
}
