...
export function AddTransactionSheet({
  ...
  return <Sheet title="New Transaction" onClose={onClose}>
    ...
    <div className="mb-4 flex gap-2 overflow-x-auto">{sources.map((item) => <button type="button" key={item.id} onClick={() => setSourceId(item.id)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm ${sourceId === item.id ? "bg-white text-black" : "glass text-white"}`}>{item.source_name}</button>)}</div>
    ...
    <button type="button" onClick={() => setShowCategories(true)} className="glass mb-5 flex w-full items-center justify-between rounded-2xl p-4 text-left text-sm">{effectiveCategory || "Choose category"}{suggestedCategory && !categoryTouched && <span className="text-xs text-[#FFD60A]">Suggested</span>}</button>
